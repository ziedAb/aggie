// Receives reports from a content service and stores in a circular queue.
// If reports are arriving too quickly, the queue will overflow and the oldest reports will be overwritten first, due to
// nature of circular queue. If this happens, it is written to the log.
// Provides reports one-at-a-time via calls to fetchNext.

var CircularQueue = require('./circular-queue');
var EventEmitter = require('events').EventEmitter;
var util = require('util');
var _ = require('underscore');
var logger = require('../logger');
// var json = require('json');

// options.source - The source for which this bot will be fetching.
// options.contentService - The contentService which this bot governs.
var Bot = function(options) {
  this.source = options.source;
  this.contentService = this.contentService || options.contentService;
  console.log(this.contentService);
  this.type = this.contentService.fetchType;
  this.queue = new CircularQueue(options.queueCapacity);
  this.enabled = false;
  this.incomingEventName = 'report';
  // Variable to determine whether the Bot needs to log dropped reports.
  this._logDrops = {timeout: true, notEmpty: true};

  this._reportHandler = this._reportHandler.bind(this);
  this._warningHandler = this._warningHandler.bind(this);
  this._errorHandler = this._errorHandler.bind(this);
};

util.inherits(Bot, EventEmitter);

Bot.prototype.start = function() {
  console.log("At Bot.prototype.start of "+this.contentService);
  if (this.enabled) return;
  this.enabled = true;
  this.contentService.on(this.incomingEventName, this._reportHandler);
  this.contentService.on('warning', this._warningHandler);
  this.contentService.on('error', this._errorHandler);
};

Bot.prototype.logDrops = function() {
  if (_.all(_.values(this._logDrops))) {
    this._handleError('warning', new Error('Queue full, reports being dropped. Monitor queue status for updates.'));
    this._logDrops = {timeout: false, notEmpty: false};
    var self = this;
    setTimeout(function() {
      self._logDrops.timeout = true;
    }, 60000);
    this.once('notEmpty', function() {
      self._logDrops.notEmpty = true;
    });
  }
};

Bot.prototype.stop = function() {
  console.log("At Bot.prototype.stop of "+this.contentService);
  this.enabled = false;
  this.contentService.removeListener(this.incomingEventName, this._reportHandler);
  this.contentService.removeListener('warning', this._warningHandler);
  this.contentService.removeListener('error', this._errorHandler);
  this.removeAllListeners('error');
};

// Fetch next available report in the queue
Bot.prototype.fetchNext = function() {
  // Return if queue is empty
  console.log("At Bot.prototype.fetchNext of "+this.contentService);
  if (this.isEmpty()) return;

  var report_data = this.queue.fetch();

  // Notify of queue being empty
  if (this.isEmpty()) {
    this.emit('empty');
  }
  
  console.log("Before returning from "+this.contentService+" with report content of \n"+report_data);
  return report_data;
};

Bot.prototype.isEmpty = function() {
  return this.queue.isEmpty();
};

Bot.prototype.clearQueue = function() {
  this.queue.clear();
  this.emit('empty');
};

Bot.prototype._reportHandler = function(reportData) {
  if (this.enabled) {
    
    console.log("Within _reportHandler of "+this.contentService);
    if (this.isEmpty()) this.emit('notEmpty');

    // Need to add _source foreign key reference here b/c ContentService doesn't know about Source.
    reportData._source = this.source._id;
    reportData._media = this.source.media;
    reportData._sourceNickname = this.source.nickname;

    var drops = this.queue.drops;
    this.queue.add(reportData);

    // If the last report was dropped, we may need to log this.
    if (this.queue.drops > drops) this.logDrops();

    this.emit('report', reportData);
  }
};

Bot.prototype._warningHandler = function(warning) {
  self._handleError('warning', warning);
};

// TODO _errorHandler calls _handleError! what terrible naming
Bot.prototype._errorHandler = function(error) {
  self._handleError('error', error);
};

// Log errors
Bot.prototype._handleError = function(type, error) {
  var message = error instanceof Error ? error.message : error;

  this.source.logEvent(type, message);
  logger[type](error);
};

module.exports = Bot;
