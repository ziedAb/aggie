...
BotMaster.prototype.init = function(callback) {
  ...
  console.log("Within botmaster.init");
}
...
BotMaster.prototype.addListeners = function(type, emitter) {
  switch (type) {
    case 'source':
      ...
      console.log("Logging from botmaster, addListeners, case: source");
      break;
    case 'fetching':
      this._addSettingsListeners(emitter);
      console.log("Logging from botmaster, addlisteners, case: fetching");
      break;
  }
};
...
BotMaster.prototype.start = function() {
  console.log("botmaster.start. Listing all bots");
  // console.log(this.bots);
  var self = this;
  this.enabled = true;
  this.bots.forEach(function(bot) {
    console.log("calling self.enable(bot) for each bot within botmaster. Bot: "+bot);
    self.enableBot(bot);
  });
};
...
BotMaster.prototype.stop = function() {
  console.log("within botmaster.stop");
  var self = this;
  this.enabled = false;
  this.bots.forEach(function(bot) {
    console.log("disabling bot within botmaster. this.bots.foreach. Bot: "+bot);
    self.disableBot(bot);
  });
};
...
BotMaster.prototype.kill = function(bot) {
  console.log("within botmaster.kill");
  if (bot) {
    // Stop bot to ensure unbinding of listeners
    console.log("Disabling this bot, within botmaster. Bot: "+bot);
    this.disableBot(bot);
    // Remove bot instance from list of bots
    console.log("Something might be messing up here.");
    console.log("Attempting to remove bot instance from list of bots");
    var index = this.bots.indexOf(bot);
    console.log(index);
    if (index > -1) this.bots.splice(index, 1);
  } else {
    // Kill all bots
    console.log("Within else of attempting to kill all bots. botmaster");
    console.log(this.bots);
    for (var i in this.bots) {
      console.log(this.bots[i]);
      this.disableBot(this.bots[i]);
      this.bots[i] = null;
      delete this.bots[i]; // Doesn't this just delete null?
    }
    this.bots = [];
  }
};
...
BotMaster.prototype.enableBot = function(bot) {
  ...
  console.log("attempting to enable bot: "+bot);
...
BotMaster.prototype.disableBot = function(bot) {
  console.log("Attempting to disable bot: "+bot);
  if (!this.enabled) return; //this may prevent infinite countdown of SMSGh.
  bot.stop();
};
